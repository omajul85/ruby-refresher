...
def select_elements_starting_with_vowel(array)
	array.select{ |elem| ['a', 'e', 'i', 'o', 'u'].include? elem[0] }
end
...
def remove_nils_and_false_from_array(array)
	array.compact.reject { |elem| elem == false }
end
...
def get_first_half_of_string(string)
	string.length.even? ? string.slice(0..string.length/2 - 1) : string.slice(0..string.length / 2)
end
...
def longest_word_in_array(array)
	array.sort_by{ |word| word.length }.reverse.first
end
...
def get_all_letters_in_array_of_words(array)
	array.map(&:chars).flatten.sort.uniq
end
...
def is_a_2014_bank_holiday?(date)
	[Time.new(2014,12,26), Time.new(2014,12,25), Time.new(2014,8,25), Time.new(2014,5,26), 
		Time.new(2014,5,5), Time.new(2014,4,21), Time.new(2014,4,18), Time.new(2014,1,1,)].include?(date)
end
...
def count_words_of_each_length_in_a_file(file_path)
	hash = Hash.new(0)
	f = File.read(file_path).gsub!(/[^0-9A-Za-z\s]/, '')
	f.split.map { |word| hash[word.length] += 1 }
	hash
end
